...
/*   Updated: 2024/02/08 13:21:33 by uclement         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_color lightning(t_material m, t_shape shape, t_light l, t_tuple p, t_tuple eyev, t_tuple normalv, bool in_shadow)
{
	...
	if (m.is_pat == TRUE)
	{
		color = stripe_at_object(m.pat, shape, p);
	}
	else
		color = m.color;
	effective_color = mul_color(color, l.color); // combine surface color with light color / intensivity
